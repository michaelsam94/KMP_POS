...
class PaymentUseCasesTest {
    ...
    private val fakeTransactionRepo =
        object : TransactionRepository {
            override fun observeAll(): Flow<List<Transaction>> = emptyFlow()

            override suspend fun getById(id: String) = savedTransactions[id]

            override suspend fun getByOrderId(orderId: String) = savedTransactions.values.firstOrNull { it.orderId == orderId }
    ...
    @Test
    fun `processPayment - creates transaction with correct amount and change`() =
        runTest {
    ...
    @Test
    fun `processPayment - marks order as PAID`() =
        runTest {
            val order = confirmedOrder().also { fakeOrderRepo.save(it) }
            val useCase = buildUseCase()
    ...
    @Test
    fun `processPayment - fails when tendered amount is insufficient`() =
        runTest {
            val order = confirmedOrder().also { fakeOrderRepo.save(it) }
            val useCase = buildUseCase()
    ...
    @Test
    fun `processPayment - fails on empty order`() =
        runTest {
            val emptyOrder =
                Order(
                    id = "o2",
    ...
    @Test
    fun `processPayment - discount reduces total correctly`() =
        runTest {
            val order = confirmedOrder().also { fakeOrderRepo.save(it) }
            val useCase = buildUseCase()
