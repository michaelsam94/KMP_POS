...
class CartUseCasesTest {
    ...
    @Test
    fun `addItem - adds new product to empty cart`() =
        runTest {
    ...
    @Test
    fun `addItem - fails when quantity is zero`() =
        runTest {
            val useCase = AddItemToCartUseCase(fakeRepo)
            val order = emptyOrder().also { fakeRepo.save(it) }
    ...
    @Test
    fun `removeItem - removing non-existent product leaves cart unchanged`() =
        runTest {
            val useCase = RemoveItemFromCartUseCase(fakeRepo)
            val order =
                emptyOrder().copy(
    ...
    @Test
    fun `updateQuantity - updates to new quantity`() =
        runTest {
    ...
    @Test
    fun `updateQuantity - quantity 0 removes the item`() =
        runTest {
            val useCase = UpdateCartItemQuantityUseCase(fakeRepo)
            val order =
                emptyOrder().copy(
    ...
    @Test
    fun `updateQuantity - negative quantity returns failure`() =
        runTest {
            val useCase = UpdateCartItemQuantityUseCase(fakeRepo)
            val order = emptyOrder().also { fakeRepo.save(it) }
    ...
    @Test
    fun `createOrder - creates order in DRAFT with empty cart`() =
        runTest {
