...
class ProductRepositoryImplTest {
    ...
    private fun product(id: String = "p1", barcode: String = "111") = Product(
        id = id, barcode = barcode, name = "Espresso", description = "Double shot",
        price = 15.0, category = "Beverages", stockQuantity = 100
    )

    @Test
    fun `upsert and getById returns product`() = runTest {
    ...
    @Test
    fun `getByBarcode returns correct product`() = runTest {
    ...
    @Test
    fun `observeAll emits product after upsert`() = runTest {
    ...
    @Test
    fun `upsert with same id updates the record`() = runTest {
        repository.upsert(product())
    ...
    @Test
    fun `delete removes product from store`() = runTest {
        repository.upsert(product())
    ...
    @Test
    fun `upsertAll inserts multiple products`() = runTest {
